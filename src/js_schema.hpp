...
struct Schema {
    ...
    static void append_property(ObjectSchema &, Property&&);
...
Property Schema<T>::parse_property(ContextType ctx, ValueType attributes, std::string property_name, ObjectDefaults &object_defaults) {
    ...
    if (type == "bool") {
        ...
        if (!Value::is_valid(property_object)) {
            throw std::runtime_error("Backlink property must specify 'objectType' and 'property'");
        }
...
template<typename T>
void Schema<T>::append_property(realm::ObjectSchema &object_schema, realm::Property &&property) {
...
ObjectSchema Schema<T>::parse_object_schema(ContextType ctx, ObjectType object_schema_object, ObjectDefaultsMap &defaults, ConstructorMap &constructors) {
    ...
    if (Value::is_array(ctx, properties_object)) {
        ...
        for (uint32_t i = 0; i < length; i++) {
            ...
            append_property(object_schema, parse_property(ctx, property_object, property_name, object_defaults));
        }
    }
    else {
        ...
        for (auto &property_name : property_names) {
            ...
            append_property(object_schema, parse_property(ctx, property_value, property_name, object_defaults));
        }
    }
...
typename T::Object Schema<T>::object_for_property(ContextType ctx, const Property &property) {
    ...
    switch (property.type) {
        ...
        case realm::PropertyType::LinkingObjects:
            ...
            break;
        default:
            type = string_for_property_type(property.type);
            break;
    }
    ...
    static const String property_string = "property";
    if (!property.link_origin_property_name.empty()) {
